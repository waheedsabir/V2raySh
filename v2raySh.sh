...
# Root
[[ $(id -u) != 0 ]] && echo -e "\n 哎呀……?使用 ${red}root ${none}用??行 ${yellow}~(^_^) ${none}\n" && exit 1
...
*)
	echo -e " 
	哈哈……?? ${red}辣??本${none} 不支持你的系?。 ${yellow}(-_-) ${none}

	?注: ?支持 Ubuntu 16+ / Debian 8+ / CentOS 7+ 系?
	" && exit 1
	;;
esac
...
_sys_timezone() {
	...
	echo "已?你的主机?置?Asia/Shanghai??并通?systemd-timesyncd自?同步??。"
	echo

	if [[ $IS_OPENVZ ]]; then
		echo
		echo -e "你的主机?境? ${yellow}Openvz${none} ，建?使用${yellow}v2ray mkcp${none}系列??。"
		echo -e "注意：${yellow}Openvz${none} 系????法由??机?程序控制同步。"
		echo -e "如果主机??跟??相差${yellow}超?90秒${none}，v2ray??法正常通信，??ticket?系vps主机商?整。"
	fi
}
...
v2ray_config() {
	...
	while :; do
		...
		echo
		echo "?注1: 含有 [dynamicPort] 的即?用??端口.."
		echo "?注2: [utp | srtp | wechat-video | dtls | wireguard] 分???成 [BT下? | ??通? | 微信??通? | DTLS 1.2 ?据包 | WireGuard ?据包]"
		echo
		read -p "$(echo -e "(默???: ${cyan}TCP$none)"):" v2ray_transport
...
v2ray_port_config() {
	...
	*)
		...
		while :; do
			echo -e "??入 "$yellow"V2Ray"$none" 端口 ["$magenta"1-65535"$none"]"
			read -p "$(echo -e "(默?端口: ${cyan}${random}$none):")" v2ray_port
...
v2ray_dynamic_port_start() {
	...
	while :; do
		echo -e "??入 "$yellow"V2Ray ??端口?始 "$none"范? ["$magenta"1-65535"$none"]"
		read -p "$(echo -e "(默??始端口: ${cyan}10000$none):")" v2ray_dynamic_port_start_input
		[ -z $v2ray_dynamic_port_start_input ] && v2ray_dynamic_port_start_input=10000
		case $v2ray_dynamic_port_start_input in
		$v2ray_port)
			echo
			echo " 不能和 V2Ray 端口一毛一?...."
			echo
			echo -e " ?前 V2Ray 端口：${cyan}$v2ray_port${none}"
			error
			;;
		[1-9] | [1-9][0-9] | [1-9][0-9][0-9] | [1-9][0-9][0-9][0-9] | [1-5][0-9][0-9][0-9][0-9] | 6[0-4][0-9][0-9][0-9] | 65[0-4][0-9][0-9] | 655[0-3][0-5])
			echo
			echo
			echo -e "$yellow V2Ray ??端口?始 = $cyan$v2ray_dynamic_port_start_input$none"
			echo "----------------------------------------------------------------"
			echo
			break
			;;
...
v2ray_dynamic_port_end() {
	...
	while :; do
		echo -e "??入 "$yellow"V2Ray ??端口?束 "$none"范? ["$magenta"1-65535"$none"]"
		read -p "$(echo -e "(默??束端口: ${cyan}20000$none):")" v2ray_dynamic_port_end_input
		...
		[1-9] | [1-9][0-9] | [1-9][0-9][0-9] | [1-9][0-9][0-9][0-9] | [1-5][0-9][0-9][0-9][0-9] | 6[0-4][0-9][0-9][0-9] | 65[0-4][0-9][0-9] | 655[0-3][0-5])
			...
			if [[ $v2ray_dynamic_port_end_input -le $v2ray_dynamic_port_start_input ]]; then
				echo
				echo " 不能小于或等于 V2Ray ??端口?始范?"
				echo
				echo -e " ?前 V2Ray ??端口?始：${cyan}$v2ray_dynamic_port_start_input${none}"
				error
			elif [ $lt_v2ray_port ] && [[ ${v2ray_dynamic_port_end_input} -ge $v2ray_port ]]; then
				echo
				echo " V2Ray ??端口?束范? 不能包括 V2Ray 端口..."
				echo
				echo -e " ?前 V2Ray 端口：${cyan}$v2ray_port${none}"
				error
			else
				echo
				echo
				echo -e "$yellow V2Ray ??端口?束 = $cyan$v2ray_dynamic_port_end_input$none"
				echo "----------------------------------------------------------------"
				echo
				break
			fi
...
tls_config() {
	...
	local random=$(shuf -i20001-65535 -n1)
	while :; do
		echo -e "??入 "$yellow"V2Ray"$none" 端口 ["$magenta"1-65535"$none"]，不能?? "$magenta"80"$none" 或 "$magenta"443"$none" 端口"
		read -p "$(echo -e "(默?端口: ${cyan}${random}$none):")" v2ray_port
		[ -z "$v2ray_port" ] && v2ray_port=$random
		case $v2ray_port in
		80)
	...
	while :; do
		echo
		echo -e "??入一? $magenta正确的域名$none，一定一定一定要正确，不！能！出！?！"
	...
	echo
	echo
	echo -e "$yellow ?? $magenta$domain$none $yellow解析到: $cyan$ip$none"
	echo
	echo -e "$yellow ?? $magenta$domain$none $yellow解析到: $cyan$ip$none"
	echo
	echo -e "$yellow ?? $magenta$domain$none $yellow解析到: $cyan$ip$none"
	...
	while :; do
		...
		read -p "$(echo -e "(是否已?正确解析: [${magenta}Y$none]):") " record
		if [[ -z "$record" ]]; then
			error
		else
			if [[ "$record" == [Yy] ]]; then
				...
				echo
				echo
				echo -e "$yellow 域名解析 = ${cyan}我确定已?有解析了$none"
				echo "----------------------------------------------------------------"
				echo
				break
			else
...
auto_tls_config() {
	...
	while :; do
		...
		read -p "$(echo -e "(是否自?配置 TLS: [${magenta}Y/N$none]):") " auto_install_caddy
		if [[ -z "$auto_install_caddy" ]]; then
			...
			if [[ "$auto_install_caddy" == [Yy] ]]; then
				...
				echo
				echo
				echo -e "$yellow 自?配置 TLS = $cyan$install_caddy_info$none"
				echo "----------------------------------------------------------------"
				echo
				break
...
path_config_ask() {
	echo
	while :; do
		echo -e "是否?? 网站?? 和 路?分流 [${magenta}Y/N$none]"
		read -p "$(echo -e "(默?: [${cyan}N$none]):")" path_ask
		...
		N | n)
			echo
			echo
			echo -e "$yellow 网站?? 和 路?分流 = $cyan不想配置$none"
			echo "----------------------------------------------------------------"
			echo
			break
			;;
...
path_config() {
	echo
	while :; do
		echo -e "??入想要 ${magenta}用?分流的路?$none , 例如 /233blog , 那么只需要?入 233blog 即可"
		read -p "$(echo -e "(默?: [${cyan}233blog$none]):")" path
		[[ -z $path ]] && path="233blog"

		case $path in
		*[/$]*)
			echo
			echo -e " 由于???本太辣?了..所以分流的路?不能包含$red / $none或$red $ $none???符?.... "
			echo
			error
			;;
		*)
			echo
			echo
			echo -e "$yellow 分流的路? = ${cyan}/${path}$none"
			echo "----------------------------------------------------------------"
			echo
			break
			;;
...
proxy_site_config() {
	echo
	while :; do
		echo -e "??入 ${magenta}一?正确的$none ${cyan}网址$none 用?作? ${cyan}网站的??$none , 例如 https://liyafly.com"
		echo -e "?例...你?前的域名是 $green$domain$none , ??的网址的是 https://liyafly.com"
		echo -e "然后打?你的域名?候...?示出?的?容就是?自 https://liyafly.com 的?容"
		echo -e "其?就是一?反代...明白就好..."
		echo -e "如果不能??成功...可以使用 v2ray config 修改??的网址"
		read -p "$(echo -e "(默?: [${cyan}https://liyafly.com$none]):")" proxy_site
		[[ -z $proxy_site ]] && proxy_site="https://liyafly.com"

		case $proxy_site in
		*[#$]*)
			echo
			echo -e " 由于???本太辣?了..所以??的网址不能包含$red # $none或$red $ $none???符?.... "
			echo
			error
			;;
		*)
			echo
			echo
			echo -e "$yellow ??的网址 = ${cyan}${proxy_site}$none"
			echo "----------------------------------------------------------------"
			echo
			break
			;;
...
blocked_hosts() {
	echo
	while :; do
		echo -e "是否???告?截(?影?性能) [${magenta}Y/N$none]"
		read -p "$(echo -e "(默? [${cyan}N$none]):")" blocked_ad
...
shadowsocks_config() {
	...
	while :; do
		...
		read -p "$(echo -e "(默? [${cyan}N$none]):") " install_shadowsocks
...
shadowsocks_port_config() {
	local random=$(shuf -i20001-65535 -n1)
	while :; do
		echo -e "??入 "$yellow"Shadowsocks"$none" 端口 ["$magenta"1-65535"$none"]，不能和 "$yellow"V2Ray"$none" 端口相同"
		read -p "$(echo -e "(默?端口: ${cyan}${random}$none):") " ssport
		...
		[1-9] | [1-9][0-9] | [1-9][0-9][0-9] | [1-9][0-9][0-9][0-9] | [1-5][0-9][0-9][0-9][0-9] | 6[0-4][0-9][0-9][0-9] | 65[0-4][0-9][0-9] | 655[0-3][0-5])
			...
			if [[ $tls && $ssport == "80" ]] || [[ $tls && $ssport == "443" ]]; then
				echo
				echo -e "由于你已??了 "$green"WebSocket + TLS $none或$green HTTP/2"$none" ????."
				echo
				echo -e "所以不能?? "$magenta"80"$none" 或 "$magenta"443"$none" 端口"
				error
			elif [[ $v2ray_dynamic_port_start_input == $ssport || $v2ray_dynamic_port_end_input == $ssport ]]; then
				local multi_port="${v2ray_dynamic_port_start_input} - ${v2ray_dynamic_port_end_input}"
				echo
				echo " 抱歉，此端口和 V2Ray ??端口 ?突，?前 V2Ray ??端口范??：$multi_port"
				error
...
shadowsocks_password_config() {
	...
	while :; do
		echo -e "??入 "$yellow"Shadowsocks"$none" 密?"
		read -p "$(echo -e "(默?密?: ${cyan}233blog.com$none)"): " sspass
		[ -z "$sspass" ] && sspass="233blog.com"
		case $sspass in
		*[/$]*)
			echo
			echo -e " 由于???本太辣?了..所以密?不能包含$red / $none或$red $ $none???符?.... "
			echo
			error
			;;
		*)
			echo
			echo
			echo -e "$yellow Shadowsocks 密? = $cyan$sspass$none"
			echo "----------------------------------------------------------------"
			echo
			break
			;;
...
shadowsocks_ciphers_config() {
	...
	while :; do
		echo -e "??? "$yellow"Shadowsocks"$none" 加密?? [${magenta}1-${#ciphers[*]}$none]"
		...
		echo
		read -p "$(echo -e "(默?加密??: ${cyan}${ciphers[6]}$none)"):" ssciphers_opt
		...
		[1-7])
			...
			echo
			echo
			echo -e "$yellow Shadowsocks 加密?? = $cyan${ssciphers}$none"
			echo "----------------------------------------------------------------"
			echo
			break
			;;
...
install_info() {
	...
	if [[ $v2ray_transport == [45] ]]; then
		...
		if [[ $ban_ad ]]; then
			echo
			echo -e "$yellow ?告?截 = $cyan$blocked_ad_info$none"
		fi
		if [[ $is_path ]]; then
			echo
			echo -e "$yellow 路?分流 = ${cyan}/${path}$none"
		fi
	elif [[ $v2ray_transport -ge 18 ]]; then
		echo
		echo -e "$yellow V2Ray 端口 = $cyan$v2ray_port$none"
		echo
		echo -e "$yellow V2Ray ??端口范? = $cyan${v2ray_dynamic_port_start_input} - ${v2ray_dynamic_port_end_input}$none"
...
domain_check() {
	...
	if [[ $test_domain != $ip ]]; then
		echo
		echo -e "$red ??域名解析??....$none"
		echo
		echo -e " 你的域名: $yellow$domain$none 未解析到: $cyan$ip$none"
		echo
		echo -e " 你的域名?前解析到: $cyan$test_domain$none"
		echo
		echo "?注...如果你的域名是使用 Cloudflare 解析的?..在 Status 那里?一下那??..?它?灰"
		echo
		exit 1
	fi
}
...
install_v2ray() {
	...
	if [[ $local_install ]]; then
		if [[ ! -d $(pwd)/config ]]; then
			echo
			echo -e "$red 哎呀呀...安?失?了咯...$none"
			echo
			echo -e " ?确保你有完整的上? 233v2.com 的 V2Ray 一?安??本 & 管理?本到?前 ${green}$(pwd) $none目?下"
			echo
			exit 1
		fi
	...
	if [[ ! -d /etc/v2ray/233boy/v2ray ]]; then
		echo
		echo -e "$red 哎呀呀...克隆?本??出?了...$none"
		echo
		echo -e " ?馨提示..... ???自行安? Git: ${green}$cmd install -y git $none 之后再安?此?本"
		echo
		exit 1
	fi
...
pause() {
	...
	read -rsp "$(echo -e "按$green Enter 回?? $none??....或按$red Ctrl + C $none取消.")" -d $'\n'
	echo
}
do_service() {
	if [[ $systemd ]]; then
...
install() {
	if [[ -f /usr/bin/v2ray/v2ray && -f /etc/v2ray/config.json ]] && [[ -f $backup && -d /etc/v2ray/233boy/v2ray ]]; then
		echo
		echo " 大佬...你已?安? V2Ray 啦...?需重新安?"
		echo
		echo -e " $yellow?入 ${cyan}v2ray${none} $yellow即可管理 V2Ray${none}"
		echo
		exit 1
		...
		echo "  如果你需要??安?.. ?先卸??版本"
		echo
		echo -e " $yellow?入 ${cyan}v2ray uninstall${none} $yellow即可卸?${none}"
		echo
		exit 1
	fi
	v2ray_config
...
uninstall() {
	...
	if [[ -f /usr/bin/v2ray/v2ray && -f /etc/v2ray/config.json ]] && [[ -f $backup && -d /etc/v2ray/233boy/v2ray ]]; then
		...
		echo -e "
		$red 大胸弟...你貌似毛有安? V2Ray ....卸????哦...$none

		?注...?支持卸?使用我 (233v2.com) 提供的 V2Ray 一?安??本
		" && exit 1
	fi
...
*)
	echo
	echo -e " 你?入的???? <$red $args $none> ...??是什么鬼啊...?本不??它哇"
	echo
	echo -e " ??辣??本?支持?入$green local / online $none??"
	echo
	echo -e " ?入$yellow local $none即是使用本地安?"
	echo
	echo -e " ?入$yellow online $none即是使用在?安? (默?)"
	echo
	exit 1
	;;
esac
...
while :; do
	echo
	echo "........... V2Ray 一?安??本 & 管理?本 by 233v2.com .........."
	echo
	echo "?助?明: https://233v2.com/post/1/"
	echo
	...
	echo
	if [[ $local_install ]]; then
		echo -e "$yellow ?馨提示.. 本地安?已?用 ..$none"
		echo
	fi
